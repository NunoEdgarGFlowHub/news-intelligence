...
(function(app) {
  ...
  app.WatsonModel.prototype._convertOptionsToQuery = function(options, queryStr) {
    var queryStrInternal = queryStr;
    if (!queryStrInternal) {
      queryStrInternal = '';
    }
    var sep = queryStrInternal.length ? '&' : '';

    for (var attr in options) {
      if (options.hasOwnProperty(attr)) {
        queryStrInternal += sep + encodeURIComponent(attr) + '=';
        queryStrInternal += encodeURIComponent(options[attr]);
        sep = '&';
      }
    }
    return queryStrInternal;
  };
